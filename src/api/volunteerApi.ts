...
export const getVolunteers = async ({ page, size, keyword, regionCode, date, categories }: any) => {
  ...
  return await http.get(`/api/shelter/volunteer/list?page=${page??0}&size=${size??10}&keyword=${keyword??''}&regionCode=${regionCode??0}&date=${date??''}&categories=${categoriesParam}`)
}